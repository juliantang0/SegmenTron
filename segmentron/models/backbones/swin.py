...
This code is adapted from https://github.com/SwinTransformer/Swin-Transformer-Object-Detection/blob/master/mmdet/models/backbones/swin_transformer.py
 with minimal modifications.  # noqa
--------------------------------------------------------
Swin Transformer
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn = WindowAttention(
            dim,
            window_size=_to_tuple(self.window_size),
            num_heads=num_heads,
            qkv_bias=qkv_bias,
            qk_scale=qk_scale,
            attn_drop=attn_drop,
...
class PatchEmbed(nn.Module):
    ...
    Args:
        patch_size (int or tuple[int, int]): Patch token size. Default: 4.
...
class SwinTransformer(nn.Module):
    ...
    Args:
        ...
        norm_layer (type[LayerNorm]): Normalization layer. Default: nn.LayerNorm.
    ...
    def __init__(
        ...
        if self.ape:
            pretrain_img_size = _to_tuple(pretrain_img_size)
            patch_size = _to_tuple(patch_size)
            patches_resolution = [
...
def swin_t_224(norm_layer=nn.LayerNorm):
...
@BACKBONE_REGISTRY.register()
def swin_s_224(norm_layer=nn.LayerNorm):
    model = SwinTransformer(
        pretrain_img_size=224,
        window_size=7,
        embed_dim=96,
...
@BACKBONE_REGISTRY.register()
def swin_b_224(norm_layer=nn.LayerNorm):
    model = SwinTransformer(
        pretrain_img_size=224,
        window_size=7,
        embed_dim=128,
...
@BACKBONE_REGISTRY.register()
def swin_b_384(norm_layer=nn.LayerNorm):
    model = SwinTransformer(
...
@BACKBONE_REGISTRY.register()
def swin_l_224(norm_layer=nn.LayerNorm):
    model = SwinTransformer(
        pretrain_img_size=224,
        window_size=7,
        embed_dim=192,
...
@BACKBONE_REGISTRY.register()
def swin_l_384(norm_layer=nn.LayerNorm):
    model = SwinTransformer(
        pretrain_img_size=384,
        window_size=12,
        embed_dim=192,
