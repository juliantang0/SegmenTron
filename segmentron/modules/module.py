...
from .basic import _ConvBNReLU, SeparableConv2d, _ConvBN, _BNPReLU, _ConvBNPReLU, _ConvGNReLU
from ..config import cfg

__all__ = ['_FCNHead', '_ASPP', 'PyramidPooling', 'PAM_Module', 'CAM_Module', 'EESP', 'BasePixelDecoder']


class _FCNHead(nn.Module):
    def __init__(self, in_channels, out_channels, inter_channels=None, norm_layer=nn.BatchNorm2d):
        super(_FCNHead, self).__init__()
        self.inter_channels = in_channels // 4 if inter_channels is None else inter_channels
        self.block = nn.Sequential(
            nn.Conv2d(in_channels, self.inter_channels, 3, padding=1, bias=False),
            norm_layer(self.inter_channels),
            nn.ReLU(inplace=True),
            nn.Dropout(0.1),
            nn.Conv2d(self.inter_channels, out_channels, 1)
        )
...
class PyramidPooling(nn.Module):
    def __init__(self, in_channels, out_channels=None, sizes=(1, 2, 3, 6), norm_layer=nn.BatchNorm2d, **kwargs):
        ...
        for size in sizes:
            ...
            self.convs.append(_ConvBNReLU(in_channels, self.out_channels, 1, norm_layer=norm_layer, **kwargs))
...
class BasePixelDecoder(nn.Module):
    ...
    def __init__(self, in_channels, feat_channels, out_channels):
        ...
        for i in range(0, self.num_inputs - 1):
            lateral_conv = _ConvGNReLU(
                in_channels[i],
                feat_channels,
                kernel_size=1,
                num_groups=32
            )
            output_conv = _ConvGNReLU(
                feat_channels,
                feat_channels,
                kernel_size=3,
                stride=1,
        ...
        self.last_feat_conv = _ConvGNReLU(
            in_channels[-1],
            feat_channels,
            kernel_size=3,
            padding=1,
    ...
    def forward(self, inputs):
        ...
        """
        y = self.last_feat_conv(inputs[-1])
